...
export default function StudentDetails({
  ...
  const attendanceHistory = useMemo(() => {
    ...
    return dates.map(date => {
      ...
      return { date, dayName: dayOfWeekName, monthName, dayNum, isPresent, isAbsent, isNA, isMarked };
    });
  }, [student.attendance]);
        ...
        {activeTab === "Attendance" && (
          <div className="flex flex-col gap-4 animate-fadeIn">
            ...
            <div className="flex flex-col gap-3 mt-1">
              {attendanceHistory.map(({ date, dayName, monthName, dayNum, isPresent, isAbsent, isNA, isMarked }) => (
                <div
                  key={date}
                ...
                >
                  ...
                  <div className="flex gap-1.5 sm:gap-2">
                    <button
                      ...
                      className={`px-2 py-1.5 text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider rounded-xl border flex items-center gap-1 transition-all ${
                        !isAdmin ? "cursor-default opacity-85" : "cursor-pointer"
                      } ${
                        isMarked && isPresent
